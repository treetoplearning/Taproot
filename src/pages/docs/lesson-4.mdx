...
So last week we talked about a lot of the mathematical operations that we can do as programmers. Though, a lot of of programs we wrote only had to do with numbers and basic operations. As programmers, we aren't always directly working with numbers. Sometimes we want to find a yes or a no, or more specifically, a True or a False value for a question that we might have. So, today we are going to learn about a tool to do just that called booleans. 
