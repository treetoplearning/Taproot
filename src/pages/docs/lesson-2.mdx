...
For today's lesson we are going to learn are going to continue our discussion about some of the basic fundamentals of coding so that we can get better sense of what's to come in the next couple of weeks. Sound good? Our goals for this lesson are:

<CheckboxList lines={["The student understands what variables are and how to create them.", "The student understands that variables are used to store data, and data types dictate the type of data stored in a variable. Additionally, students understand that Python automatically assigns the type of variables for them. ", "The student is able to assign different values to a variable and is able run basic checks on value of a variable.", "The student understands that a string is a datatype. The students also understands that strings can be combined through a process called string concatentaion.", "The student understands that they can use variables to perform many operations. These operations include but are not limited to adding integers, concatenating strings, and checking whether a boolean is true."]}/> 
...
Last week we were introduced to what coding was, and the process that the code we write becomes instructions to be followed by the computer. This week we are going to dive even deeper and learn about some more tools that coders use write programs. You'll recall that last week we talked about [vocabulary](#vocabulary), which are a way to assign persistent values, so those values can be referenced later. Let's get right to some question so we can learn more.

 - *Why do you think it might be useful to be able to reference an already-known computer value at different points while coding?* It saves us time from recomputing known values and makes our code shorter and simpler.
...
As a quick side note, variables generally should always start with a letter of the alphabet or an underscore. Also, in Python, would use a casing called [snake_case](#vocabulary) for variable names where things that we name in our code are only named with lowercase letters and underscores. A good way to remember to use underscores and lowercase in snake case is to think of a snake, small and low to the ground. I have a few examples for you here:
...
| `snake_case` | `the type of casing that is used for naming Python variables, which consists of all lowercase with an underscore seperating words` |
| `integer` | `the type of casing that is used for naming Python variables, which consists of all lowercase with an underscore seperating words` |
| `strings` | `a data type of are sequences of individual characters which can be words, sentences, or individual unique characters on your keyboard` |
